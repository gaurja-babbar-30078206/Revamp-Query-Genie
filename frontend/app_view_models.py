...
from app_models import DataFormat
...
def ingest_multi_doc(file_list, embed_model, embed_model_name):
    ...
    for uploaded_file in file_list:
        ...
        print(insight_json)
        print("END")
        insight_json_dict[file_name_without_ext] = insight_json  # Store in dictionary
        blog(f"Json created of ---> ")
        blog(insight_json)

        ## creating vector store and retrievers
        ...
        retriever_dict[file_name_without_ext] = (
            compression_retriever  # Store in dictionary
        )

    return retriever_dict, insight_json_dict
...
from app_models import ComparisonOutput
